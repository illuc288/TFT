...
Also this proof relies on the fact that since the functor
    ...
    \begin{enumerate}
        \item $\Zf(\adjincludegraphics[valign=c, height=0.3cm, raise=0.1\baselineskip,
    set vsize={1cm}{0cm}]{images/Classification 1TFT/identityplus.png})=id_{X}$
        \item$\Zf(\adjincludegraphics[valign=c, height=0.3cm, raise=0.1\baselineskip,
    set vsize={1cm}{0cm}]{images/Classification 1TFT/identityminus.png})=id_{X^\vee}$
        \item $\Zf(\adjincludegraphics[valign=c, height=0.5cm, raise=0.3\baselineskip,
    set vsize={1cm}{0cm}]{images/Classification 1TFT/evaluation.png})=ev_{X}$, hence $X^\vee\otimes X\xrightarrow{ev_{X}} \unit_{\cat}$ and more precisely for $v\in X\in\Vect_{k}$: $ev_{X}:(X,\lambda)\mapsto \lambda(v)$
        \item $\Zf(\adjincludegraphics[valign=c, height=0.5cm, raise=0.3\baselineskip,
    set vsize={1cm}{0cm}]{images/Classification 1TFT/coevaluation.png})=coev_{X}$, hence $\unit_{\cat}\xrightarrow{coev_{X}}X\otimes X^\vee$ and more precisely for $\lambda\in k\in\Vect_{k}$, via the canonical isomorphism $X\otimes X^\vee\cong\operatorname{End}(X)$, $coev_X$ corresponds to taking the trace
        \item $\Zf(\adjincludegraphics[valign=c, height=0.6cm, raise=0.2\baselineskip,
    set vsize={1cm}{0cm}]{images/Classification 1TFT/circle.png})=coev_{X^\vee} \circ ev_{X}$, hence $\unit_{\cat}\xrightarrow{coev_{X^\vee} \circ ev_{X}}\unit_{\cat}$, and, following the cases for $\Vect_{k}$ treated in the previous two points, to taking the trace of the identity matrix, i.e. the dimension of $X$
    \end{enumerate}
    % \includegraphics[scale=0.15]{images/Classification 1TFT/identityplus.png}, images/Classification 1TFT/coevaluation.png , images/Classification 1TFT/circle.png
    ...
    \begin{itemize}
        ...
        \item what $ev_X$, $coev_X$, $ev_X$ and $coev_X$ are and combinations thereof.
    \end{itemize}
